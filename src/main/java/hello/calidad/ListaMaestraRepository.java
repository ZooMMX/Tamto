...
public interface ListaMaestraRepository extends CrudRepository<DocumentoInterno, Long> {
    @Query("SELECT count(di) FROM DocumentoInterno di WHERE di.enabled = 1")
    public Long countEnabled();

    @Query(value = "SELECT * FROM documento_interno WHERE proxima_revision >= CURDATE() AND proxima_revision < (DATE_ADD(CURDATE(), INTERVAL 1 MONTH));", nativeQuery = true)
    public List<DocumentoInterno> getDocumentosProximaRevision();

    @Query(value = "SELECT * FROM documento_interno WHERE proxima_revision <= CURDATE();", nativeQuery = true)
